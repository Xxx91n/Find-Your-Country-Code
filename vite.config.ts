...
// 票 38（A-011）：版本真源唯一 —— package.json 的 version 是唯一声明点，
// 构建时注入产物 `// @version`，消除手写版本副本的漂移面（闸门见 verify-38）。
const configDir = dirname(fileURLToPath(import.meta.url));
const pkgVersion = JSON.parse(readFileSync(join(configDir, 'package.json'), 'utf8')).version;

export default defineConfig({
  build: { outDir: 'dist', emptyOutDir: true },
  plugins: [monkey({ entry: 'src/main.ts', userscript: {
    name: { '': 'Find-Your-Country-Code', 'zh-CN': '快速选择你的手机号国家区号' }, namespace: 'https://github.com/Xxx91n/Find-Your-Country-Code', version: pkgVersion,
