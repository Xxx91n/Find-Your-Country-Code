...
## monitor 例外的控制点修订（2026-09-18 · Cycle-8 D-010 采纳）

> 形式：**追加**，不改动上方各节原文（含「monitor 类例外」与「联动裁定」原文）。

- **原文（保留）**：monitor 例外「联动裁定（2026-09-17，T-02 自查结论落地）」末段登记「**残留**：站内 sync 开关状态须由维护者登入 GF 脚本页人工确认；**确认前不得声称 D-002 送达链已闭合**（D-018：本轮无发版）」。
- **修订（D-010 采纳行业对标结论）**：发布确认的**主控制点由「站内 sync 开关的人工确认」改为「仓库侧版本门」**——即 ① 同步源只跟 **release 分支或 tag**（**绝不跟 `main`**）；② 以 **`@version` 变更**为唯一发布门；③ 确认人 = 能向该 ref 合并代码的人（单人项目即维护者本人）。
- **站内人工确认降为验证步**：原残留不再是**控制手段**，而是**事后验证**（核对平台侧版本历史与仓库侧 tag 是否一致）。
- **原残留结论继续有效**：「**确认前不得声称 D-002 送达链已闭合**」**不变**。
- **D-002 负向不变**：仍禁 CI 主动写 GF、禁伪造 session cookie 的推送 hack、禁据此宣布弃用 GF 渠道；D-002③「保留拉取式同步兜底」与本次修订**同构**（本次只是**指定了 ref**）。
- **风险红线（新增登记）**：**绝不让自动同步跟随「维护权可能易手的上游」**（The Great Suspender 教训：维护权转移后经自动更新通道推送恶意代码，200 万用户中招）。
- **参考**：`docs/adr/0014-ci-evidence-loop-and-landing-model.md` · `.scratch/cycle8-grill/research/q6-distribution-sync-confirmation.md`（11 条来源）· 账本 D-010
