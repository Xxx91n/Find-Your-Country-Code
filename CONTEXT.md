...
**受管工件区**：
内容为**人写/裁决的产物**、或被构建/验证链消费的仓库目录（如 `tests/`、`docs/adr/`、`.scratch/architecture-recovery/`），其内容**不可抛弃、必须保持 Git 跟踪**；与**可抛弃现场**（工具可再生成的产物、探针草稿）相对。判据不是目录名，而是「可再生成性」＋「是否被门禁读取」。
_Avoid_: 临时目录、scratch 区、草稿目录（当它们指代受管内容时——这三个词只在指**真可抛弃**的 `draft/` 时成立）
## 验证与发布

**验收阶梯**：
对「工具是否生效」的断言深度分层，五级：L0 静默健康（无未捕获异常，永不单独算生效）、L1 元素已注入、L2 交互可驱动、L3 写入结果正确且持久、L4 用户反馈出现。每级同时规定层归属（哪一类页面承载、是否阻断合入），是「该证据能证明到哪一级」的统一标尺，而非用例清单。注：L0–L4 指**断言层级**，与「信号层」的 L0–L4（评分信号瀑布）同名不同轴，引用时须显式区分。
_Avoid_: 测试用例集、覆盖清单（那是清单，不是标尺）

**形态语料**：
以真实站点表单**形态**为对象固化的自有可控页面语料（含镜像页与结构骨架），承载端到端形态覆盖与断言，是「真实覆盖面」的可度量指标；原始快照不入库。
_Avoid_: 校准语料（那是阈值标定的另一语义轴）、样本集（无形态与覆盖面语义）

**镜像页**：
真实页面经去品牌 / 去追踪 / 内容替换后的自有可控副本，是形态语料的断言主力面（确定性最高）；原始快照归档于库外，不入版本库。
_Avoid_: 快照、页面副本（无「自有可控 + 去品牌」语义）

**结构骨架**：
从真实页面抽取、仅保留结构而不保留内容的长期结构基线，用于形态改版的**确定性结构比对**（非像素比对）。
_Avoid_: 快照、DOM 快照（泛称，无结构基线语义）

**发布门**：
发布流水线的放行条件：真实站点层最近一次运行必须为绿，或失败已被显式确认并立票，否则不出包。与「PR 门控」分层——PR 不因真实站点层的 flaky 而阻断合入，发布则必须已通过真实环境验证。
_Avoid_: 发布审批（那是人工动作；发布门是机器前置条件）

（决策记录见 `docs/adr/0010-release-gate.md`。）

## 诊断

**诊断面**：
把「检测 → 注入 → 面板 → 填充」全链路的判定过程外显给人（面板视图）与机器（机器可读输出）的能力面，两个出口同源于一份数据，使「未生效」可被归因到具体一层。
_Avoid_: 日志（无结构化判定语义）、调试开关（那是门控手段）

**判定记录**：
全链路各层判定点留下的结构化记录（步骤 / 判定 / 依据 / 时序），是诊断面的唯一事实来源；记录中的失败原因必须指向已验证因果，不得猜测。
_Avoid_: 错误日志、事件日志（无「逐层判定 + 已验证因果」语义）
