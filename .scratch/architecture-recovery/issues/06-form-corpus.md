...
**R1 终态（代码 tip `e5a0a542`；文档终态 `f2304687`，见报告 R1-13/14）**：`Verify Ticket 06 (form corpus)` run `35090400764` / `35090650159` **success** · `Typecheck` `35090400773` / `35090650206` success · `Engine Gates` `35090400767` / `35090650139` success · `Lockfile Regen` `35090400742` / `35090650151` success · `E2E` `35090400731` / `35090650166` **1 failed / 134 passed**（唯一红 = `entry-access.spec.ts:42` 票 37，非本票；本票 18 例全绿）。
**自曝事故（已修）**：R1 首次提交 `6d0462ae` 按 hunk ID 提交致三文件**仅部分 hunk 入库**（探测主体改动与门 S9 未入），提交信息与实物不符（P-7 类）；已以文件 ID 重新提交补齐（`vrl`），核验门 209-0、`but status` 无残留。教训见报告 `### R1-11`
