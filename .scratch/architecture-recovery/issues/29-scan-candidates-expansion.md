...
**Status:** done（4/5 验收销项；第 5 项 E2E 的 CI 取证受跨分支安装面共因阻塞，见文末 D-29c）
...
- [ ] E2E 全量绿；证据锚 commit sha + CI run ID —— **本地全量 80 passed（含本票 4 条）**，但 CI 侧 E2E run `34693447646` 失败于 `npm install` 安装阶段（13s 即红，非测试失败）：`.npmrc`（票 31 D-31a）与票 34 R1 内联 `--legacy-peer-deps` 均在独立分支、未进入本票基线；票 28 已记录「E2E 在 27/28/29 三条 W2 分支同因失败于 npm install」。属跨分支共因，非本票回归。CI 侧替代证据：Verify Ticket 29 run `34693186397`/`34693447618` 绿、Typecheck run `34693447628` 绿。任一安装面修复合入 main 后重跑即可补齐本项（偏离点 D-29c）