...
| 2026-09-17 | S7(票11) | 用 `String.raw` 模板承载含 `\uXXXX` 的内容去写工具文件（结构门 / spec），转义被**字面保留** ⇒ 源码出现 `\u2550` 等不可读序列（文件仍能运行，但文本型结构门按中文匹配时全部失配，两文件共 1811 处）；同因使 3 条断言假红，返工一轮 | 写 JS/TS 工具文件时：中文与换行用**真实字符 / 真实换行**（普通模板字面量，或行数组 `join` 拼接），**只有正则反斜杠**才需转义；写完必须三项校验：① 文件内 `\uXXXX` 字面序列计数 == 0；② `node --check` 通过；③ 跑一次文本型结构门自证（教训：工具也要先自证） |

| 2026-09-17 | S7(票10) | 门内**替身保真度缺口**可同时制造「守卫静默失效」与「假绿」：`verify-ticket-05` 的 BroadcastChannel 替身 `fn({ data: msg })` **缺 `origin`**（真实 message 事件恒携带），使被测 origin 守卫在门内退化为 `undefined !== undefined` 恒放行 —— 守卫**从未被行使**；又因替身不按结构化克隆投递，接收方 `_normRulesDoc(msg.rules)` 就地截断的恰是**发送方**的 `_rulesCache`，故 S4「上限生效」的绿是**别名旁路效应**而非实现保证（判据：仅补 `origin` → `100/100`；再补克隆 → `got=513` 复红） | 门内 mock 必须与被模拟平台对象**逐字段对齐**（尤其是**被断言路径所消费的字段**）；替身引入「按引用投递」等非平台语义前先确认无断言依赖其副作用；行为面修复后必须复跑全部门，并对每一个计数变化给出机制级解释（禁「跑一下看看」） |

| 2026-09-17 | S7(票10) | **跨帧写入的读侧竞态**在本地不可见、在 CI 必红：跨帧写入是异步链（顶层 `ui/index.ts:820-821` 先 `postMessage(FRAME_FILL_MSG)` 再**同步** `_closePopup()`，子帧 message 任务 + `Fill.run` 在其后执行）⇒ 「面板已 detach」**不等于**「写入已完成」。票 10 在**两处**各踩一次：① 自身密封用例用一次性读值（E2E run 35126041454 红，本地 6/6 绿）；② 真实站点层 `live-smoke.mjs` 写后一次性读（真实目标 CI 红而本地全阶梯全通过）。票 06 R1 已登记同类（`selectCountry` 返回后立即读值）—— **同一类错误第三次出现** | 跨帧 / 异步写入后的一切读取一律用**条件等待**（密封层 `expect.poll` / web-first 重试断言；live 层采样式有界等待），禁一次性读；「本地绿、CI 红」且失败点落在**写后读取**时，先查读侧竞态再查功能缺陷；判定读侧修复是否「为凑绿」的硬标准：**修复前的失败是否确定性**（确定性失败 ⇒ 重试不可能伪造绿） |
