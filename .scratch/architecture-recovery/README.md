...
- **❗R-2 范围补正（同日实物复核，**扩大**）**：不是「缺一个导出」，而是 cch/02 的 `src/ui/index.ts`（1,159 行）被扫入**票 03 诊断面整层**（`_diagLevel`/`_diagLayer` 态 · 摘要条 · 独立诊断视图 · 过滤器 · `deps.Diag` 读面；`grep "票 03"` 命中 25+ 处），而同分支 `src/config.ts` **零** DIAG 常量、`src/types.ts` **零** Diag 类型、`src/main.ts` **零** Diag 注入、`src/diag/` 目录**不存在** ⇒ 该分支是**嵌合体**。即便补导出，类型门仍红且诊断面永远渲染空。**修法必须是剔除被扫入的整层**。sha 亦修正：先前记的 `61f3ebe7` 是 rebase 前的孤儿提交（不属于任何分支），分支实际 sha = `06273351`。
...
| 02 | 设置面收口 | A-026 · A-027 | **⚠️ 存在源码层面问题（R-2 待返工）** — W1 的「无源码返工」结论**已回溯作废**（基于并集树，未做逐分支构建）：**cch/02 在 CI 上构建失败**（**构建步**；`ui/index.ts` 被扫入票 03 诊断面**整层**，而 config/types/main/`src/diag/` 四者全缺 ⇒ **嵌合体分支**，非「缺一个导出」）· 修复版启动器 `prompts/02-settings-surface-fix.md` · 原 W1 描述： — 8/8 声明实物属实；闸门 **33/0**、规格 **8 passed** 实测复现；`_applyLocaleText` 已删；**CI 证据待补（P-2）**；报告行号漂移待补正（P-5） | `research/window-reports/02-settings-surface-report.md` | W1 |
