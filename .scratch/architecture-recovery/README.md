...
| **W5** | 08, 10, 11 | 08←07；10←07；11←07 | 三票同波（互不堆叠，可并行；**10 为 P0 优先**） | 阶段 B：失效驱动修复（A-031 · A-032） ＋ srcdoc 帧跨帧 origin 校验修复（A-034，P0） ＋ ITI 形态 L3 判据判定（A-035，P1） |
...
| 10 | 修复 `about:srcdoc` 帧跨帧 origin 校验误判 | A-034 | **ready-for-agent（P0）** — 票 07 全阶梯运行暴露的真实缺陷；首脑已独立复现确证（srcdoc 帧 `location.origin` = `"null"`，`window.origin` 保留真实 origin）；阻断真实站点层 L3/L4 转绿 ⇒ 影响 ADR-0010 发布门「必须绿」判据 | `research/window-reports/10-srcdoc-origin-fix-report.md` | W5 |
| 11 | 判定 ITI 形态下 L3 的正确可观测判据 | A-035 | **ready-for-agent（P1）** — L3（读宿主 `input.value`）对 ITI 接管字段不成立，而同帧 L4 toast 自认成功（L3/L4 相悖）；**判定前不得以改判据方式消红** | `research/window-reports/11-iti-l3-criterion-report.md` | W5 |
