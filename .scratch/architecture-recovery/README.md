...
- **量化**：`git ls-files .scratch` **397 → 210**（199 现役流程文件[含本票报告] + 11 在途票产物）
...
- **❗R-2（新，源码层面）：cch/02 在 CI 上构建失败** —— `src/ui/index.ts (4:41): "DIAG_TRACE_PREF" is not exported by "src/config.ts"`（CI run 35089321289，**Build userscript** 步，未到测试步）。逐分支矩阵：cch/02 = 导出 **0** / 引用 **2** ✗，其余四支均 1/3 ✓。根因：`06273351`（fix(cch-02)）切走了**票 03 的在途改动**却未带配套导出。
- **❗R-2 范围补正（同日实物复核，**扩大**）**：不是「缺一个导出」，而是 cch/02 的 `src/ui/index.ts`（1,159 行）被扫入**票 03 诊断面整层**（`_diagLevel`/`_diagLayer` 态 · 摘要条 · 独立诊断视图 · 过滤器 · `deps.Diag` 读面；`票 03` 出现 **8** 次 —— 我先前记的「25+ 处」是**多模式 grep 的行数误计**，已在 R2 复核 §六 自纠），而同分支 `src/config.ts` **零** DIAG 常量、`src/types.ts` **零** Diag 类型、`src/main.ts` **零** Diag 注入、`src/diag/` 目录**不存在** ⇒ 该分支是**嵌合体**。即便补导出，类型门仍红且诊断面永远渲染空。**修法必须是剔除被扫入的整层**。~~sha 亦修正：先前记的 `61f3ebe7` 是 rebase 前的孤儿提交（不属于任何分支），分支实际 sha = `06273351`~~ ⇒ **该句已被 R2 复核 §六 推翻**（`61f3ebe7` 实为远端 cch/02 的 fix 提交、被 5 个远端分支包含；孤儿是 `06273351`）。
...
- **❗R-3（新）：entry-access 垂直居中 CI-only 红**（`:55` 期望 <30px 实测 **36.5px**；本地 7 passed）。归因链（我独立复现 + CI **步骤级**证据）：cch/47/48 **success** → cch/02 红于**构建步**（35089321289）→ **cch/03 首个测试级红**（35089332673，Run E2E）→ cch/01（35089341835）/ cch/05（35089351200）/ cch/06（35089360299）逐支继承。建议立修复票。
- **过程违规（未追认）**：P-12 票 02 提交切走票 03 **整层诊断面**致分支不可构建；**P-13 已升级为方法级约束** —— 六个工作分支**本地 tip ≠ 远端 tip**（01: 9601b34b≠18fbf99b · 02: 3a936cec≠b95f672f · 03: 9e99c68b≠23229ed9 · 05: 0851ba32≠ede691c3 · 06: d54cb334≠1e661ed2；cch/04 未推），故「本地跑绿」不构成「推送后跑绿」的证据，跨支终态一律以 CI 为准；P-14 **E2E 在全部已推工作分支上均 failure → 无分支可合入**；P-2 部分缓解（远端现 7 支 cch/*，cch/04 仍未推）。
...
### R2 轮次复核（票 02 去污染移交包）（首脑，2026-09-16）

详见 `research/cycle6-r2-review.md`。**结论：R2 交付全部属实、可复现；落地未完成；本人 §7 三处事实错误已自纠。**

- **产物完整性**：三件（补丁 23,634 B / `ui/index.ts` 43,319 B / `i18n.ts` 6,110 B）尺寸·行数·SHA-256 与自述**逐字命中**；补丁为**纯删除式**（ui 删 298 行=1160→862，i18n 删 4 行=59→55，数学自洽）。
- **可施性**：`patch -p1 --dry-run` 施于**当前** cch/02 内容 → 无 fuzz / 无 reject；应用结果与两个全文**逐字节相等**（sha256 双命中）。
- **硬验收（我独立实跑，隔离树）**：修复前 `build` **exit 1** 且错误串与 CI `35089321289` **逐字一致** → 修复后 `build` **exit 0**（132.73 kB）· `typecheck` **exit 0/0 错** · 门 02 **33 PASS/0 FAIL** · 门 42 **42 PASS/0 FAIL** · 全量 E2E **103 passed / 1 failed**（唯一红 = `entry-access:53` 居中 33.12 = R-3）。
- **静态正确性**：`票 03`/`DIAG_`/`diag`/`PanelView` **全 0**；`票 02` 标记 **11 处全保留**；类型 import 行与基线**同一行**；基线 `50383e3c` blob `42265f2b` / 770 行 / 洁净（`_applyLocaleText` 仍在）。**污染仅限 2 文件**（窗口定位到 `i18n.ts`，**我漏了**）。
- **剔除范围判定**：票 02 改动集内 4 个测试文件（`entry-access` +16 · `helpers/userscript.ts` +22 · `iframe.e2e` +4 · `locale-switch` +19）**均为票 02 自身语义适配**（菜单 2→3 条 / `cch-menu-settings` 断言 / id 原地更新替身 / 三选一控件 + 旧按钮回归钉）⇒ 补丁只动 2 个 src 文件**正确且完整**。
- **❗本人 §7 三处事实错误（窗口反证成立，已自纠）**：① sha 归属**说反**（`61f3ebe7` 实为**远端 cch/02 的 fix 提交**、被 5 个远端分支包含；孤儿是 `06273351`）——根因是我用了 `git branch --contains`（**仅本地 ref**），而本地六支已被 rebase ⇒ **P-13 陷阱咬到我自己**，判 sha 归属须用 `git branch -a --contains` / 对远端 ref 做 merge-base；② `types.ts:6` **张冠李戴**（该 import 行实为 `ui/index.ts:6`，types.ts:6 是注释）；③ `票 03`「25+ 处」**口径错误**（多模式 grep 行数，实际 8）。
- **❗R-3 归因修正（我的假设被推翻）**：`_pos()` 在基线与 cch/02 **逐字节相同**（票 02 未改）；剔除摘要条后 `:53` 仍以 33.12 失败（CI 为 36.5 ⇒ 随树而变）⇒ 真因在 `_pos()` 的 `anchor=null` 分支用**调用时** `pop.offsetHeight` 居中、而面板随后因 `_render()` 填充行数据**继续增高**（中心随 Δh/2 下移）——**基线代码的时序/内容敏感问题，非票 03 引入**。
- **❗我自己的跑法缺陷（方法教训，如实登记）**：首次隔离树实跑得 **92/12** 并准备判窗口「103/1 不实」——**该判定是错的，错在我**：未按窗口做法链入依赖（`tests/server.mjs` 从 `ROOT/node_modules` 供 `/vendor/react|react-dom|vue|intl-tel-input`、`/gen/react19/` 取 `tests/vendor/react19/node_modules`，两者均被 `node_modules/` 忽略、不入 `git archive`）⇒ 框架夹具 404、Vue 未挂载（DOM 残留 `{{ JSON.stringify(...) }}`）⇒ 11 项连带失败。补齐两个链接后复跑即 **103/1**，与窗口**完全一致**。**纪律**：隔离检出必须显式链入**全部**依赖根（含嵌套 vendor 根）；把差异判为对方失真前先自查跑法。
- **落地阻断（待裁定）**：GitButler 报 `depends on cch/03-diagnostics-surface`；依 GitButler 技能第 157–159 行，这是**原子性拒绝**，官方恢复路径（`but move --above` / `--anchor`）会把票 02 变成**依赖票 03**，与本票目标相反 ⇒ 不可用。栈结构：`cch/47 → cch/48 → cch/02 → cch/03 → cch/01 → cch/05 → cch/06` 同栈。**建议选项 ①**：自顶向下卸下 06/05/01/03 → 使 cch/02 成栈顶 → 写入修复 → 提交 → 自底向上装回（必要时 `but resolve`）；风险=cch/03 的 `ui/index.ts` hunk 可能冲突 + 卸装期间工作树缩减影响并行 agent ⇒ **须授权**。
- **过程违规（未追认）**：P-15 报告/issue 照抄本人错误 sha 归属与计数；P-16 未标注「复核对象=远端 tip」与本地已 rebase 的差异；P-2 cch/02 仍未推送；P-14 E2E 全支 failure 无分支可合入。
- **正面记录**：窗口**先复核后动手**、**定位到 2 文件含我漏掉的 `i18n.ts`**、**反证主 Agent 两处事实**、**未以补 config 导出为修法**、**临时写主工作区后逐字节还原并自曝**、**被依赖机制阻断后不硬来而呈报移交** —— 六项均符合纪律。
...
**frontier（重算）**：W4 = 票 07 ✅ **复核通过** → **下一波（W5）可开工：`08` · `10`（P0）· `11`（P1）** —— 三票 Blocked by 均为票 07，同波并行、互不堆叠；W6 = 票 09 被 08 阻塞。

### W5 复核（票 08 · 10 · 11）（首脑，2026-09-17）

详见 `research/cycle6-wave5-review.md`。**结论：三票实现全部属实 · 无源码层面返工**；偏差集中在**呈报完整性**与**本地/远端同步**。

- **票 10（P0）✅ 闭环**：门 **34/0** · `16485782` = 9 文件/+516 −9 逐字命中 · `SELF_ORIGIN` 四处 + 回退不放宽 · 跨票改动仅替身 1 行（断言面零改动）· **真实站点层 `live-codepen-pen-fullpage` CI 上 L0–L4 全绿**（修复前 L3!/L4!）· CI 6 run 全 success @ `62f2292a` · E2E `143 passed`。
- **票 08 ✅**：门 **35/0**（我实跑）· 回归矩阵 12 道门逐项吻合 · 语料/夹具实物齐（manifest 56 例 · 16/16 指纹自洽）· `src/` 改动无后门 · 无断言弱化。3 处**报告表述偏差**（非源码）已追加「附：首脑补正」。
- **票 11 ✅（判据变更判为合规）**：门 **62/0** · `0f195075` = 11 文件/+975 −20 逐字命中 · 普通字段判据逐字保留、ITI 分支 fail-closed · **合规铁证**：钉版库全文**仅 1 处 `dispatchEvent`**（`:788`）⇒ 旧原生事件断言对 ITI **确定性不可满足**（假红，非时序）⇒ 收敛有据、非消红；反证：若动机为消红，`live-codepen-pen-fullpage` 会被一并转绿，而它在新判据下**仍红且归因他票**。2 处**未呈报影响面** + P-19 已追加「附：首脑补正」。
- **账本维度**：A-031 ✅（三形态入语料 + 引擎只加护栏）· A-032 ✅（1 处字面偏差：两子形态为同文件两元素）· A-034 ✅ **达成**（P0 闭环）· A-035 ✅（2 处影响面缺口）。
- **过程违规（未追认）**：**P-19** 票 11 §8「本票不产生远端写」**与实物不符**（远端有该支且 5 门全绿）· **P-20** 本地 tip ≠ 远端 tip（`cch/08` `d0430821` vs `747763fd`；`cch/11` `f9d60760` vs `8a27ad34`）⇒ P-13 类复现，且票 11 的 amend 自纠未传播到远端 · **P-21** 跨票改动（票 08 改 `14-lib-engine.mjs`；票 11 改 `live-smoke.mjs` + `site-manifest.json` + `WORKFLOW.md` 1 行，均自曝）· **P-22** 票 08 E-4 非快进推送改写 6 个祖先远端 ref（自曝）；票 10 E-10 经**用户逐次授权**后改用单 ref `git push` 并给出「每次推送前后 `ls-remote` 全量 diff 仅一行变动」的隔离证据。**正面**：票 10 自曝并修复两处竞态、挖出门保真度缺口（决定性 A/B 对照）；票 11 用钉版库源码证伪旧判据 —— 三项均属本周期最高质量取证。
...
| 10 | `prompts/10-srcdoc-origin-fix.md` |
| 11 | `prompts/11-iti-l3-criterion.md` |
| 12 | `prompts/12-rules-limit-fidelity.md` |
