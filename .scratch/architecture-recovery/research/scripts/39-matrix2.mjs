...
const combos = [
  { id: 'full+chromeUA', channel: 'chromium', ua: CHROME_UA, flag: false },
  { id: 'full+defUA+flag', channel: 'chromium', ua: undefined, flag: true },
  { id: 'shell+chromeUA+flag', channel: undefined, ua: CHROME_UA, flag: true },
];
for (const c of combos) {
  const o = { headless: true };
  if (c.channel) o.channel = c.channel;
  if (c.flag) o.args = ['--disable-blink-features=AutomationControlled'];
  ...
  const rec = { combo: c.id, challenged: null, title: null, tWrapperMs: null, previewWrapper: 0, pageErrors: 0 };
  try {
