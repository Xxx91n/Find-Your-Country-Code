...
| **D-29c** | 本票 **E2E 未能在 CI 取证** | E2E 失败于 `npm install` 安装阶段（13s 即红，非测试失败）：`.npmrc`（票 31 D-31a 产物）与票 34 R1 的内联 `--legacy-peer-deps` 均在**独立分支**、未进入本票基线。票 28 记录同一现象：「E2E 在 27/28/29 三条 W2 分支同因失败于 npm install 安装阶段」。**属跨分支共因，非本票回归**。证据替代：本地全量 80 passed + verify-29 CI 两度绿 + Typecheck CI 绿。**后续已绕开取证**：按票 27 D-27a 先例，在本票自有 `verify-29.yml` 内复刻 E2E 步骤（显式 `--legacy-peer-deps`，零接触共享 `e2e.yml`），run 34695833032 的 E2E 作业 **70 passed**，见 D-29h。 |
...
| **D-29i** | 分支推送被**票 34 在途提交的 lockfile 冲突**阻塞（用户裁定：停止推送，等待对方自解） | 票 27 把 `cch/27` 挂到 `cch/34` 之上，票 34 的「CI 再生 lockfile 入库」提交（`a7ac6e4`）因此进入本票必经祖先链；该提交在 `package-lock.json` 上有 **32 处冲突**——与我 D-29b 的 lockfile 重生成属同一件事的重复劳动（同一 `package.json` 的机械再生）。`but push` 直接拒绝：「the push would include 1 conflicted commit」。此时不含票 27 的栈又缺 `L1_ATTR_PHRASE_SCORE`（Typecheck 必红），即**两条路都走不通**。本票已四次重建堆叠恢复绿灯（`dfa5fb4`/`dd863fa`/`3d21a12`/`388b674`），每次都被他窗口 push 重算基址打回，且每次重算都会改写 `cch/27`/`cch/28` 的远端头。**用户裁定：停止推送，不代他票裁决冲突。**当前远端头 `dca9dc4` 为红（Typecheck 失败，纯栈卫生问题，非代码缺陷）；绿灯证据已完整留档（run 34696053977：gate 27/27 + E2E 70 passed + Typecheck 绿 + 性能 `maxMs=17`）。**解锁配方**（票 27/34 冲突解决后一次性执行）：`but move cch/29-scan-candidates-expansion --above cch/27-detection-coverage-floor` → `but push cch/29-scan-candidates-expansion` → 复验三步（`git merge-base --is-ancestor origin/cch/27 <29头>` + `git show origin/cch/29:src/config.ts \| grep L1_ATTR_PHRASE_SCORE` 为 1 + CI 三作业全绿）。**根本建议**：把 `L1_ATTR_PHRASE_SCORE` 这类跨票共享常量提到 W1 基线（如随票 32 合入）或尽快合入 main，否则 W2 三条分支将持续互相牵连；同时建议同一栈只由**一个窗口**负责推送。 |
| **D-29h** | 以**本票自有 workflow 复刻 E2E 步骤**取证，未修改共享 `e2e.yml` | 共享 `E2E` workflow 在本票基线仍是裸 `npm install`，ERESOLVE 预存红（13s 即红，非测试失败）。修复分属两条未合流分支（票 34 R1 内联 `--legacy-peer-deps`、票 31 D-31a `.npmrc`）。若在本票改共享 `e2e.yml`，会与票 34 的同一文件改动争用。按**票 27 D-27a 先例**，改在 `verify-29.yml` 新增 `ticket-29-e2e` 作业（node 22 + `npm install --legacy-peer-deps` + playwright install + `npm run build` + `npx playwright test`）。结果 run **34695833032**：E2E 作业 **70 passed（26.4s）**、gate 作业 27/27，并输出 `[cch-29 perf] maxMs=14`（CI 环境优于本地 73ms）。**建议**：安装面修复合入 main 后，可删除该作业回归共享 E2E；在合流前，各票自建 E2E 作业是可复用的取证范式。 |
...
| **定态头**（全栈内容齐全：`detect` 实现 3 处 + `tabindex` 形态描述符 + `L1_ATTR_PHRASE_SCORE` 常量；栈序 `32 → 28 → 27 → 29`） | **`dd863fa`** | Verify Ticket 29 **34695451445** / Typecheck **34695451449** / E2E **34695451437** | 绿 / 绿 / 红（安装阶段，见 D-29c）。**推送纪律**：本栈每次 `but commit` 后须重建堆叠（`but move 29 --above 27`）再 `but push`，否则 GitButler 重算基址会甩掉票 27/28（三次实证：e83514c、4d572d2、d7ffc6e 均因缺栈上依赖而红）。随后的 docs-only 提交 sha 以分支头为准，内容等同本行。 |
| **E2E 取证作业**（`verify-29.yml` 新增 `ticket-29-e2e`，同票 27 D-27a 口径，零接触共享 `e2e.yml`） | `3d21a12` | Verify Ticket 29 **34695833032**（作业 `Ticket-29 E2E` **70 passed（26.4s）**、作业 `Ticket-29 gate` 27/27）；共享 `E2E` **34695833031** | 绿 / 红（共享 E2E 仍卡安装阶段，见 D-29c/D-29h）。CI 性能探针 `[cch-29 perf] maxMs=14`（1000 节点含 50 可聚焦容器），远低于 350ms 红线 |
