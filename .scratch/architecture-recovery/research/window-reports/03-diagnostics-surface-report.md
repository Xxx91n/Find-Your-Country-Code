...
## 10 补正（Cycle-7 / 票 T-06，决策 D-009，日期 2026-09-17 Asia/Singapore）

> 本节为**追加补正**，上文原文（§1–§9）逐字节保留，未改写、未删除。

**被补正的主张**：§2 复现基线表首行称「`src/` 内 `console.` 调用点 = **0**」，表下注亦称「基线 `console.` 计数为 **0** 是 A-028 的核心事实：全项目零可观测性」。

**实测：该主张不成立。** 截至本补正，`src/` 内 `console.*` 调用点**恰好为 1**，位于本票新增模块自身的失败自保分支（`put()` 的 catch 块）：

```js
// src/diag/index.ts（补正前，原文引用）
try { console.warn('[cch] diag write failed'); } catch {}
```

即：环形缓冲写入抛错时，仍向宿主页面控制台打印了一条**未门控**的 `warn`。§2 的「0」计数与实际不符；原文保留，以本补正为准。

**补正动作（票 T-06 / 决策 D-009）**：

1. **改为可门控，非删除**：该输出改走本模块**既有的**诊断开关 —— `createDiag()` 内的 `let traceFlag`（由 `traceOn()` / `setTrace(on)` 读改，经诊断面 / `cch-menu-diag` GM 菜单命令驱动）；**未**新增全局开关或配置常量。
2. **脚本名前缀**：由 `[cch]` 细化为 `[cch][diag]`，与文件既有短前缀风格一致。
3. **失败可观测性未删除**：信息仍可达（门控开启时可见），防御性 `try {} catch {}` 外壳保留；失败仍写入内部环形缓冲／诊断面，cycle6 D-012 的单采集源双 serializer 契约不变。

补正后代码：

```js
// src/diag/index.ts（补正后，put() 的 catch 块内）
// 诊断面自保：写入失败降级为门控告警（traceFlag 开启时才打印），不重试、不抛给主流程（调研 Q8：诊断不是业务逻辑）
try { if (traceFlag) console.warn('[cch][diag] write failed'); } catch {}
```

**对 §2 基线的修正**：`src/` 内 `console.*` 调用点 = **1**（非 0），且该 1 处**已门控**（默认关闭）；可复算命令：`git grep -c console. -- src/`。
