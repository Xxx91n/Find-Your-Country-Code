...
> Cycle-5 | 票: `issues/42-locale-switch.md` | 覆盖 A-xxx: **A-019**
> 上游：`spec.md`（Cycle-5）+ `decision-ledger.md`（A-011…A-025）

**必读清单（绝对路径）:**
- `D:\Aworker\mozilla\choose-your-country\.scratch\architecture-recovery\issues\42-locale-switch.md`
...
- 不新增依赖
- 存储键与收藏/规则解耦
- 若只做死导出清理，需在报告里显式声明“功能不做”

**完成定义:** issue 全部验收项勾销并各附 commit sha（只读验证命令 + 输出摘要）；报告落 `research/window-reports/42-locale-switch-report.md`；版本控制遵循 WORKFLOW §4.2。
