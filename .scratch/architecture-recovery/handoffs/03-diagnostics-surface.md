...
> Cycle-6 | 票：`issues/03-diagnostics-surface.md` | 覆盖 A-xxx：**A-028**
...
**完成定义:** issue 全部验收项勾销并各附 commit sha（只读验证命令 + 输出摘要）；报告落 `research/window-reports/03-diagnostics-surface-report.md`；版本控制遵循 WORKFLOW §4.2。
