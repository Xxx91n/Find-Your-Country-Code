...
**必读清单（绝对路径）:**
- `D:\Aworker\mozilla\choose-your-country\.scratch\architecture-recovery\issues\45-repo-process-closeout.md`
...
- 只删已确认合并的分支；不触碰 main
- 远端写操作须用户授权
- 条款不得放松“证据只认 CI run/artifact”总原则

**完成定义:** issue 全部验收项勾销并各附 commit sha（只读验证命令 + 输出摘要）；报告落 `research/window-reports/45-repo-process-closeout-report.md`；版本控制遵循 WORKFLOW §4.2。
