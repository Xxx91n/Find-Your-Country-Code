# Handoff 41 — 过程证据出仓与升塔纪律

> Cycle-5 | 票: `issues/41-process-evidence-archive.md` | 覆盖 A-xxx: **A-018**
> 上游：`spec.md`（Cycle-5）+ `decision-ledger.md`（A-011…A-025）

**必读清单（绝对路径）:**
- `D:\Aworker\mozilla\choose-your-country\.scratch\architecture-recovery\issues\41-process-evidence-archive.md`
...
- 归档不销毁（可移到仓库外或独立分支）
- 不得破坏 README 引用的路径
- 归档后逐条重解析 README/ledger 中的路径引用

**完成定义:** issue 全部验收项勾销并各附 commit sha（只读验证命令 + 输出摘要）；报告落 `research/window-reports/41-process-evidence-archive-report.md`；版本控制遵循 WORKFLOW §4.2。
