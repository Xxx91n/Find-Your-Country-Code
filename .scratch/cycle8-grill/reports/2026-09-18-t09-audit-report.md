...
2. **审计未验证项**（超出可核验范围，如实声明）：
   - 「`but` 为唯一版本控制入口、未执行裸 `git` 写操作」——**无法从当前仓库状态回溯证明**，属**信任项**；**第二轮已补取正向佐证**（见 **§10.2**）：本轮当日 HEAD reflog 仅 GitButler 操作、无裸 git 写签名；但仍不可结论，保留为信任项。
   - 「A6 force-push 的 tree 逐一相同」——**第二轮已展开并验证成立**（见 **§10.1**）。
...
## 10. 补充验证（第二轮 · 2026-09-18 审计窗口追加）

> 本节闭合 §7 声明的两项「未验证」，并记录 R3 登记结果。

### 10.1 A6 force-push 的 tree 等价性 —— **已验证，声明成立**

§7 原声明「需备份 ref 对照，本轮未展开」。第二轮已展开：

- **备份 ref 存在**：`refs/backup/main-pre-rewrite` = `b1fcf96d27cd762dc2edf2ed4e2b34e3b5116a71`。
- **三对 old → new 的 tree 逐一相同**：

| old | new | tree（两侧相同） |
|---|---|---|
| `e63e8253` | `6fbca4c7` | `361075d9f095211912c5ad2e176a79744f8f9393` |
| `337461e8` | `d856d75f` | `f79f7f206c041a28d12a4858fe0703f9bdbd560b` |
| `b1fcf96d` | `d075f01a` | `a4c9f2535c2ef08b47355246687331529bf9dda8` |

- **author / committer / 时间戳三对全部一致**（逐字段比对 `%an|%ae|%ad|%cn|%ce|%cd`）。
- **提交信息差异仅限授权的那一处**：
  - `e63e8253 → 6fbca4c7`：subject 由「**13 张票（12 完整 + T-14 部分）**」改为「**15/15 票（T-01…T-15）**」，**body 完全一致** —— 与「授权改写提交信息票数口径 13 → 15/15」**精确吻合**。
  - `337461e8 → d856d75f` 与 `b1fcf96d → d075f01a`：**subject 与 body 均完全一致**（仅 sha 变，属父链重写）。

⇒ **A6 声明（tree 逐一相同 / author-committer 保留 / 内容零变化 / 仅改一个提交信息）全部成立。**

### 10.2 「`but` 为唯一版本控制入口」—— **部分正向佐证（非结论性）**

§7 原声明「无法从仓库状态回溯证明，属信任项」。第二轮补取可观测证据：

- **HEAD = `refs/heads/gitbutler/workspace`**，HEAD 提交为 **`GitButler Workspace Commit`** ⇒ 工作区确由 GitButler 托管。
- **无进行中的裸 git 状态**：`.git/MERGE_HEAD` · `rebase-merge` · `rebase-apply` · `CHERRY_PICK_HEAD` **均不存在**；`.git/ORIG_HEAD` 存在但时间为 **2026-09-17 17:33**（**上一周期**，非本轮）。
- **本轮当日（2026-09-18）的 HEAD reflog 仅 4 条，全部为 `GitButler checkout workspace during apply-branch`**，**无任何裸 `git commit` / `reset` / `rebase` 签名**。
- **诚实边界**：HEAD reflog **不能**证明「未在任何分支上执行过裸 `git` 写」（分支 ref 的 reflog 独立于 HEAD）；且 GitButler 自身也经 git 底层写入，故无法据此断言零裸 git。⇒ 结论：**由「不可验证」升级为「有正向佐证但不可结论」**，仍列为**信任项**。

### 10.3 R3 登记结果 —— **已执行**

审计报告 §6-R3 要求的三条登记类发现已写入 `.scratch/evidence/findings-register.md`：

| 新增 | 来源 | 内容摘要 |
|---|---|---|
| **FR-12** | F-5 | 报告脚本数字为时点快照（52/6604 → 54/7409；5-5 → 6-6），复跑不复现；不得当 CI 期望值 |
| **FR-13** | F-6 | `CONTRIBUTING.md` 未反映 ADR-0014/0016（遗留漂移，非本轮引入） |
| **FR-14** | F-7 | Wilson 与 OC 的 α 口径不一致（双侧 95% vs 单侧 5%；保守 42%，非缺陷） |

**不变量校验**：改动 **仅 1 文件 / +3 −0**；`FR-01…FR-11` · `FC-01…FC-08` · `FN-01/FN-02` **原样未动**；UTF-8 无 BOM / 纯 LF。

### 10.4 第二轮后的未闭合项（最终）

**唯一实质未闭合项 = F-3（T-1 / T-2 / T-3 三项裁定待用户逐条追认）。**
登记类：FR-12 / FR-13 / FR-14（open，到期 2027-03-31）＋ 原有 FR-01…FR-11。

---

## 11. F-3 闭合记录（2026-09-18 · 用户逐条追认）

> 本节为**追加**；§0–§10 为审计时点快照，保留不动。

- **§4-F-3 的要求**：D-008 / D-009 / D-011 的负向前置（「T-1 未决前不得改 landing 模型」/「T-2 未决前本票不得开工」/「T-3 未决前不得改写 Cycle-7 D-018」）由执行窗口自裁解除，**须用户逐条追认**。
- **用户 2026-09-18 指令原文**：「确认确认确认。给你权限。之后push，之后删除所有已经合并的分支」。
- **逐条追认结果**：**T-1 确认 · T-2 确认 · T-3 确认**（落账见 `decision-ledger.md`「追认 — T-1 / T-2 / T-3」段）。
- **⇒ F-3 闭合**。§10.4 的「唯一实质未闭合项」**已消除**。
- **剩余未闭合项**：仅**登记类** FR-01…FR-14（open，到期 2027-03-31）。
- **授权边界（用户同批授予）**：push / land · main 分支保护（FR-08）· 建站点级语料（FR-11）· 备份 ref 清理（FR-03，**破坏性**）。
