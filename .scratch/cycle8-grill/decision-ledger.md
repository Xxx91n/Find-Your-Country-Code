...
- 已确认条目：**3**（D-001 · D-002 · D-003 = current）｜revised：0｜stale：0｜deferred：0｜**pending：2**（D-004 · D-005）
- 本问覆盖：Cycle-8 输入面 = **已定**（B）｜并行在途融合 = **已定且已执行闭环**｜`.scratch/` 定性 = **已定**（D-003）｜**Q3 作用域 = 调研已出，待拍板**
- **审计遗留 9 项进度**：A6 待裁定 · **B⑦ 已拍板（D-003，待落 ADR 文）** · B⑧⑨⑩ 待裁定 · C⑪⑫ 待裁定 · **C⑬ 已闭环（D-002）** · **C⑭（CRLF 漂移）待裁定**
- 待决（frontier）：**D-004 / D-005 拍板** → B⑧⑨⑩ / C⑪⑫⑭ / A6；`audit-closeout` §5 的 Q3–Q8；新登记 **F-1…F-8**
