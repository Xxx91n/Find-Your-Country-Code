...
- 本问覆盖：目标函数 = **已定**（A+C）｜**A 侧发版口径 = 已定**｜**C 侧收口债口径 = 已定**（register 回写 / 9 条签核 / ADR 形式债 / CONTRIBUTING 同步 / FR-03 删 / lockfile 对齐）
- 待决（frontier）：**执行授权面**（`workflow_dispatch` 补跑 · bump · `push` · `land` · 备份 ref 删 · 清账 land）；**本轮交付单位与验收面**
