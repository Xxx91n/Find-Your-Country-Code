...
- 已确认条目：**18**（D-001…D-018 = current）｜revised：0｜stale：0｜deferred：0｜待拍板：**0**
- 本问覆盖：Cycle-7 目标函数 = **已定**（A+B）；Q2（锐评 5 工单）= **已定稿**；Q3（backlog `B-5…B-15` ＋ B-16）= **已定稿**；Q4（B 侧能力集）= **已定稿**；Q5（A/B 排序与耦合）= **已定稿**；Q6（交付单位与验收面）= **已定稿**（不发版，累积再发）
- 待决（frontier）：**A10（D-005 乙级三项）本轮是否纳入**（唯一剩余；收口形态按既有惯例默认执行）
