...
- 已确认条目：**17**（D-001…D-017 = current）｜revised：0｜stale：0｜deferred：0｜待拍板：**0**
- 本问覆盖：Cycle-7 目标函数 = **已定**（A+B）；Q2（锐评 5 工单）= **已定稿**；Q3（backlog `B-5…B-15` ＋ B-16）= **已定稿**；Q4（B 侧能力集）= **已定稿**；Q5（A/B 排序与耦合）= **已定稿**
- 待决（frontier）：**本轮交付单位与验收面**（唯一剩余）
