...
.cch-diag-exp:hover{border-color:rgba(15,118,110,.45);color:#0f766e}`;
  ...
  open(target: AnyEl | null, kind: FillKind | null, anchor: AnyEl | null, opts: { remoteSource?: Window | null } = {}): void {
    ...
    sumBtn.type = 'button'; sumBtn.id = 'cch-diag-tg';
    sumBtn.setAttribute('data-i18n', 'diagOpen');
    sumBtn.textContent = t('diagOpen');
...
<button type="button" class="cch-fav${fav ? ' on' : ''}" data-code="${c.code}" data-iso="${c.iso}" title="${fav ? t('rmFav') : t('addFav')}">${fav ? '★' : '☆'}</button>`;
  ...
  _renderDiag(): void {
    ...
    tgl.addEventListener('click', e => {
      ...
      const on = !D.traceOn();
      D.setTrace(on);
      try { localStorage.setItem(DIAG_TRACE_PREF, on ? '1' : '0'); } catch {}
      this._renderDiag();
    });
    hd.appendChild(chip);
    hd.appendChild(tgl);
    ...
    chk.forEach(c => {
      ...
      nm.textContent = t('diag' + c.layer.charAt(0).toUpperCase() + c.layer.slice(1));
